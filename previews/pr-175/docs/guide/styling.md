...
A staggered `animation-delay` is a natural fit here, but interpolate it as its own longhand declaration. The `animation:` shorthand resets `animation-delay` to `0s`, overriding the interpolated value.
