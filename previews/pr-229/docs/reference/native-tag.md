...
- There is no attribute form for the [`capture`, `once`, and `passive` options](https://developer.mozilla.org/en-US/docs/Web/API/EventTarget/addEventListener#options). Those require `addEventListener` from a [`<script>`](./core-tag.md#script) with an [element reference](#element-references).
