...
## Serialize the PR preview deploy across pull requests, not just per branch

`.github/workflows/preview.yml` › `concurrency` | 2026-07-30 | impact:med | effort:low

Every preview publishes to the one shared `gh-pages` branch, but the workflow's concurrency group is `preview-${{ github.head_ref }}`, which is scoped per branch. That serializes repeated pushes to a single PR while leaving concurrent runs from _different_ PRs free to push the same ref at once, so each loses the race with `! [remote rejected] gh-pages -> gh-pages (cannot lock ref 'refs/heads/gh-pages': is at <sha> but expected <sha>)`. The three built-in retries do not help, because they re-push the same stale local ref rather than re-fetching between attempts, and the job then fails with "Failed to deploy preview after 3 attempts" even though the docs change itself is fine. This fires whenever more than one PR is opened close together, which makes a batch of small docs PRs look broken. Consider a second concurrency gate keyed on the deploy target rather than the head ref (a constant group with `cancel-in-progress: false` around the publish step, so pushes queue instead of racing), keeping the existing per-branch cancel for superseded runs. Re-verify: open two PRs within about a minute and watch both `deploy-preview` jobs; one fails on the ref lock, and re-running it in isolation passes with no code change. Observed on runs 30508607718 (failed) and the same job re-run alone (passed).
