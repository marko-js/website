...
- A folder that is itself a tag (it has `index.marko`, a matching `FOLDER_NAME.marko`, or `marko-tag.json`) becomes that one tag; its other files are not crawled, so examples or helpers can sit beside it.
